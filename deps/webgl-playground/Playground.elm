...
toX : Keyboard -> Float
toX keyboard =
    (if keyboard.right || keyboard.dKey then
        1

     else
        0
    )
        - (if keyboard.left || keyboard.aKey then
            1
...
-}
game : (Computer -> memory -> List Shape) -> (Computer -> memory -> memory) -> memory -> Sub Msg -> Program () (Playground memory) Msg
game viewMemory updateMemory initialMemory sub =
    let
        { init, update } =
        ...
        view (Playground { computer, entities }) =
            Html.div []
                (viewWrap computer.screen entities)
    in
    Browser.element
        { init = \_ -> init
        , view = view
