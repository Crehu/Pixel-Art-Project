...
gameUpdate viewMemory updateMemory msg (Playground ({ visibility, memory, textures, computer, initialMemory } as model)) =
    case msg of
        Tick time ->
            let
                now =
            ...
            let
                initial =
                    if paused then
                        ...
                        False
            in
            ( Playground { model | computer = { computer | paused = paused }, memory = initialMemory }, Cmd.none )
...
updateKeyboard isDown key keyboard =
    ...
    case key of
        ...
        "KeyW" ->
            { keyboard | keys = keys, wKey = isDown }

        "KeyA" ->
            { keyboard | keys = keys, aKey = isDown }

        "KeyS" ->
            { keyboard | keys = keys, sKey = isDown }

        "KeyD" ->
            { keyboard | keys = keys, dKey = isDown }

        "KeyQ" ->
            { keyboard | keys = keys, qKey = isDown }

        "KeyE" ->
            { keyboard | keys = keys, eKey = isDown }
